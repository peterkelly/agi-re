...
room with `call_logic_var(v0)`. Menu arrow navigation consumes type-2 movement
...
3. Extend the room-switch probes from the now-matched re-entry fixture. Useful
   follow-ups are entry-boundary selector `v2`, previous-room byte `v1`,
   resource/event replay, and object-0 placement after the destination room
   logic performs its own entry setup.
...
- **Room/resource lifecycle:** `0x12` and `0x13` are broad room/state switch
  ...
  immediate and variable-selected room actions. Deeper side effects remain
  source-backed: object/resource reset, previous-room byte update, destination
  logic load, byte-variable-2 entry-boundary placement, and resource/event
  recording. `0x15`, `0x1b`, `0x1c`, `0x20`, and `0x99` also have targeted
