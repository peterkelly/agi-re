...
- Run the local compatibility suite with `AGI_GAME_DIR=PATH python3 -B -m unittest discover -s tests` when changing decoders, renderers, or resource parsers. The higher-level suite manifest/runner is `python3 -B tools/compatibility_suite.py --game-dir PATH`; it runs local tests, both mdBooks, and opcode-evidence checks by default, with QEMU smoke/broad sweeps available through explicit flags. The selected path is exported only to child commands and recorded in the JSON report; it has no implicit game default.
