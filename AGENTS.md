...
- Run the local compatibility suite with `AGI_GAME_DIR=PATH python3 -B -m unittest discover -s tests` when changing decoders, renderers, or resource parsers. The higher-level suite manifest/runner is `python3 -B tools/compatibility_suite.py --game-dir PATH`; it runs local tests, both mdBooks, and opcode-evidence checks by default, with QEMU smoke, broad, and exhaustive sweeps available through explicit flags. `--include-qemu-exhaustive` inherits smoke/broad and adds every present picture, all deterministic movement cases, and all current overlay cases. Its object probes use ten-case snapshot chunks to fit the generated DOS disk. Nondeterministic cases remain available as named exploratory probes but must not gate this layer. The selected path is exported only to child commands and recorded in the JSON report; it has no implicit game default.
- Export language-neutral visual reference bundles from completed batch or v3 behavior reports with `python3 -B tools/conformance_results.py export ...`; compare an independent engine bundle with the `compare` subcommand. Keep source-report adapters in `docs/`, while the normative bundle and canonical-frame contract remain in `spec/`.
...
- Start with `git status --short` and narrow `rg` queries over the files or
  symbols implicated by the request. Do not begin with an unbounded repository
  search or dump entire long chapters when a targeted range will answer the
  question. Increase scope only after the focused search fails.
- Read existing dirty diffs before editing. Preserve unrelated work and avoid
  repeatedly rediscovering or reprinting changes already present in the
  worktree.
- For controller playthroughs, keep one QEMU/controller session alive across
  route stages. Rebuild or reboot only when the generated interpreter, disk, or
  controller process actually needs replacement.
- Do not enable `--capture-every-cycle` or `--capture-logical-buffers` during
  routine route execution. They add a screenshot, memory reads, hashing, and
  filesystem writes at every semantic stop. Use
  `POST /v1/captures/cycle` at milestones and enable continuous capture only
  for a short interval whose per-cycle evidence is specifically required.
- Prefer one bounded semantic request over repeated cycle-level calls. Use
  transactions, guarded run-until, movement segments, waypoints, and named
  waits instead of issuing `cycles/step`, polling state, or taking screenshots
  one cycle at a time. Reuse the final state and trace slice already returned
  by the request instead of immediately fetching them again.
- Put deterministic entropy control and a short checkpoint save/restore smoke
  test before a long playthrough. Verify that restore reinstalls the correct
  semantic hook and repeats the expected random sequence before investing in
  later route stages.
- On the first unexpected modal, hook, room, or movement result, stop the
  action and inspect its bounded trace/debug state. Do not repeat the same
  input or movement attempt while the starting contract is known to be false.
- Use a verification ladder: focused unit tests first, then one generated
  artifact/runtime smoke check, then the relevant broad suite, and finally
  documentation builds after the implementation stabilizes. Do not repeatedly
  run the broadest suite while still changing the narrow fix.
- Before a broad test run, identify which private reference input its
  assertions expect. Do not export the task's game as `AGI_GAME_DIR` to a
  legacy corpus suite whose fixed hashes, counts, saves, or dictionaries are
  for another game; run focused game-neutral tests or select the suite's
  explicit documented input.
- Test deletion-of-`build/` reproducibility in a fresh temporary output tree
  rather than deleting useful local evidence. Use the smallest supported disk
  size and `--skip-vgabios` when the check concerns only disk contents, then
  verify the generated bytes by reading them back from the disk.
- When a command is still running, poll at a useful interval such as 10-30
  seconds rather than issuing many one-second waits. Keep the interval below
  the user-update deadline and report a concise status update before any
  longer wait.
