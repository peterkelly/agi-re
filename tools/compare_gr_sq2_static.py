...
CHANGED_ACTION_NOTES: dict[int, str] = {
    0x12: "GR resolves the target through the same variable/immediate operand helper used elsewhere before entering the room-switch helper; SQ2 reads the immediate byte directly in this entry.",
    0x6F: "Input-line setup differs around display-mode/input-width globals.",
    0x73: "Blocking string prompt differs around the edit-width cap; GR uses a direct 0x28 cap in the observed entry path.",
    0x76: "Number prompt enters the editor through the GR input path without SQ2's initial display-mode branch.",
    0x77: "Disable-input-line path omits SQ2's display-mode branch in the normalized entry.",
    0x78: "Enable-input-line path omits SQ2's display-mode branch in the normalized entry.",
    0x79: "Key-map table capacity changes from 0x27 in SQ2 to 0x31 in GR.",
    0x7C: "GR clears a word before entering the inventory/list helper.",
    0x7D: "Save-state path starts from a different message/data pointer source; detailed save format comparison is deferred.",
    0x80: "Restart-confirmation path has a larger local stack frame in GR.",
    0x84: "GR preserves object0 field +0x22 when it is already 4; SQ2 clears it unconditionally.",
    0x89: "Input-line refresh follows the GR input/display branch shape rather than SQ2's mode test.",
    0x8A: "Input-line erase tests visible input state directly rather than SQ2's mode/input-width branch.",
    ...
    0xA9: "Close-text-window state differs because the SQ2 input-width flag no longer exists as an active GR action.",
